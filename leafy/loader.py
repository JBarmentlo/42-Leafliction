...
class ImageLoader(Dataset):
    def __init__(self, data_folder: Path, accept_globs: bool = True):
        ...
        if self.accept_globs:
            self.image_files = list(
                Path.cwd().glob(str(self.data_folder / "**/*.JPG"))
            )
        else:
        ...
        print(
            f"Initiated loader on folder {data_folder.absolute()}. Found {len(self)} images."
        )
    ...
    def get_sub_loader(
        self, desired_fruit: str, desired_disease: Optional[str] = None
    ):
        new_image_paf_list = []
